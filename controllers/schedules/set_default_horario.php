<?php
/**
 * Set a schedule as the default one
 * 
...
try {
    ...
    // First, set all user's schedules to not default
    $stmt = $conn->prepare("UPDATE horarios SET es_predeterminado = 0 WHERE usuario_id = :usuario_id");
    $stmt->bindParam(':usuario_id', $usuario_id);
    $stmt->execute();
    
    // Then set the selected schedule as default
    $stmt = $conn->prepare("UPDATE horarios SET es_predeterminado = 1 WHERE id = :id");
    $stmt->bindParam(':id', $horario_id);
    $stmt->execute();
